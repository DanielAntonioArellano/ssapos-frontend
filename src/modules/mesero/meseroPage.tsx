...
export default function MeseroPage() {
  ...
  // Touch drag handlers para el bottom sheet
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: "#f8fafc", fontFamily: "'DM Sans', sans-serif", overflow: "hidden" }}>
      ...
      {/* ── HEADER ── */}
      <div style={{ background: "#0f172a", color: "#fff", padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        <div>
          <p style={{ margin: 0, fontSize: 11, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em" }}>Mesero</p>
          <p style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>{user?.name}</p>
        </div>

        {/* Toggle DINE_IN / DELIVERY */}
        <div style={{ display: "flex", background: "#1e293b", borderRadius: 999, padding: 3 }}>
          {(["DINE_IN", "DELIVERY"] as const).map((t) => (
            <button key={t} onClick={() => setOrderType(t)} style={{ padding: "5px 12px", borderRadius: 999, border: "none", background: orderType === t ? "#3b82f6" : "transparent", color: orderType === t ? "#fff" : "#94a3b8", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "0.2s" }}>
              {t === "DINE_IN" ? "Mesa" : "Domicilio"}
            </button>
          ))}
        </div>

        <button onClick={logout} style={{ background: "none", border: "none", color: "#94a3b8", fontSize: 12, cursor: "pointer" }}>Salir</button>
      </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, padding: "8px 16px 12px" }}>
            {categories.map((cat) => (
              <button
                ...
                style={{ padding: "6px 14px", borderRadius: 999, border: "none", background: selectedCategory === cat.id ? "#0f172a" : "#f1f5f9", color: selectedCategory === cat.id ? "#fff" : "#374151", fontSize: 13, fontWeight: 500, cursor: "pointer" }}
              >
                {cat.name}
              </button>
            ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          {filtered.map((product) => (
            <button
              ...
              style={{ background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 12, padding: "10px 8px", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, cursor: "pointer", transition: "0.15s", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}
            >
      ...
      <button
        ...
        style={{ position: "fixed", bottom: 20, left: "50%", transform: "translateX(-50%)", background: "#0f172a", color: "#fff", border: "none", borderRadius: 999, padding: "13px 28px", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 20px rgba(0,0,0,0.25)", display: "flex", alignItems: "center", gap: 10, zIndex: 50 }}
      >
        🛒 Ver orden
        {cart.length > 0 && (
          <span style={{ background: "#3b82f6", borderRadius: 999, padding: "2px 8px", fontSize: 12 }}>{cart.length}</span>
        )}
        {cart.length > 0 && <span style={{ color: "#94a3b8", fontSize: 13 }}>${total.toFixed(2)}</span>}
      </button>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 100 }}>
          ...
          >
            ...
            <div style={{ overflowY: "auto", flex: 1, padding: "0 16px 16px" }}>
              ...
              <button onClick={() => setShowCustom(!showCustom)} style={{ width: "100%", padding: "9px", border: "1.5px dashed #3b82f6", borderRadius: 10, background: "#eff6ff", color: "#3b82f6", fontSize: 13, fontWeight: 600, cursor: "pointer", marginBottom: 12 }}>
                + Producto personalizado
              </button>
              ...
              {showCustom && (
                <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                  ...
                  <button onClick={addCustom} style={{ padding: "8px 12px", background: "#0f172a", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>+</button>
                </div>
              )}
              ...
              <button
                ...
                style={{ width: "100%", padding: "14px", background: loading || cart.length === 0 ? "#e2e8f0" : "#0f172a", color: loading || cart.length === 0 ? "#94a3b8" : "#fff", border: "none", borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: loading || cart.length === 0 ? "not-allowed" : "pointer", transition: "0.2s" }}
              >
